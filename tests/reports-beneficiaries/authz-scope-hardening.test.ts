...
describe("G8 — catalog status reconciliation snapshot", () => {
  test("counts remain 63 entries; blocked hubs include VP/dean/strategic/docs", () => {
    expect(REPORT_CATALOG_ENTRIES).toHaveLength(63);
    const counts = countByStatus(REPORT_CATALOG_ENTRIES);
    expect(counts.LIVE + counts.DATA_DEPENDENT + counts.SOURCE_READY + counts.UNDER_DEVELOPMENT + counts.NOT_ACTIVATED + counts.BLOCKED).toBe(63);
    for (const code of [
