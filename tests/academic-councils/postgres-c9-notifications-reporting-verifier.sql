...
begin
  ...
  -- Schedule meeting and open intake
  v_meeting := ((public.council_schedule_meeting(v_council, 'PII Meeting', now() + interval '2 days'))->>'meeting_id')::uuid;
  perform public.council_transition_meeting(v_meeting, 'scheduled', 'intake_open', '{}'::jsonb);

  -- Member submits topic
  perform pg_temp.as_user(v_mem);
  v_topic := ((public.council_submit_topic(v_council, v_meeting, 'PII Topic', 'Topic body'))->>'topic_id')::uuid;

  -- Secretary moves to under_review
  perform pg_temp.as_user(v_sec);
  ...
  -- Chair accepts topic for agenda
  perform pg_temp.as_user(v_chair);
  ...
  -- Close intake, add topic to agenda, finalize, move to agenda_ready
  ...
  -- Issue decision assigned to member
