/**
 * LOCAL-ONLY Chrome smoke for the five B1 student services.
 * Uses file:// fixtures (no Production, no HTTP — Chrome hangs on loopback in this env).
 */
...
function main() {
  ...
  try {
    ...
    if (failed.length) {
      console.error("B1_BROWSER_SMOKE_FAIL", failed.slice(0, 20));
      process.exit(1);
    }
    console.log(`B1_BROWSER_SMOKE_PASS checks=${checks.length} viewports=360,768,1366`);
  } finally {
    try {
