...
const report = readFileSync(join(root, "docs", "B1-RELEASE-AND-FIRST-SERVICE-PREFLIGHT-PACK-01-REPORT.md"), "utf8");
const normalized = report.replace(/\s+/g, " ");
const drafts = join(root, "docs", "migration-drafts");

const ordered = [
  ["REQUEST-B1-ATOMIC-CALLER-RELEASE-EVIDENCE-STAMP-01.sql", "893a2979bad443b059bf3c0ce2f2b6ad2714dbd9333dd5b332c8c4acc64cf357"],
  ["STUDENT-REQUEST-WORKFLOW-ACTOR-AUTHORIZATION-HARDENING.sql", "0627b142b10307e72ba0c9ffd09dc4db5c02059791273f101b71463704e4f6c0"],
  ["REQUEST-PROCESSING-DOMAINS-EXPANSION-SOURCE-01.sql", "e5b5ee1cba7a39864ff07b3d95daed31b1f1a513613566b052ca3f62661a8edf"],
  ["REQUEST-B1-ATOMIC-SUBMIT-ACTION-04.sql", "a92505d71ba6e02d29b4993d10da8ff8e2f91e5fa62549a6a7efe74c1dc8b58a"],
  ["EXTERNAL-UNIVERSITY-PAYMENT-CONFIRMATION-01.sql", "da4eadb7de0a4fad8f3d5839a6b4719031a47b1b345652c5eae4ebd6fc872e4b"],
  ["STUDENT-REQUEST-SECURE-ATTACHMENTS-SOURCE-01.sql", "8487c5ae0ac8b85965de9dd08dafb934550a16e1450b0bedf4f847c5ef17849c"],
  ...
  ["REQUEST-B1-TRANSFER-SECURE-ATTACHMENT-05A.sql", "4b2124a829b3e6b57979ce50a7638d3b9b6af798f83f6a4e8772cb8ff36ceded"],
...
describe("B1 release and enrollment-suspension preflight pack", () => {
  ...
  test("proves all five services remain fail closed before migrations", () => {
    for (const service of ["enrollment_suspension", "excused_absence", "department_transfer", "final_chance", "file_withdrawal"])
  ...
  test("contains read-only before/after, smoke and rollback contracts", () => {
    for (const token of ["Read-only before queries", "Read-only after queries", "Pre-migration release smoke", "Release rollback plan", "partial apply", "protected records remain present"])
  ...
  test("does not touch deferred service scope", () => {
    ...
    expect(report).not.toMatch(/grade_statement_non_graduate|october_exam_entry_form|replacement_student_card|academic_record|graduation_certificate/);
  });
});
