...
-- Denial pattern: expected DB rejection sets v_denied; UNEXPECTED_SUCCESS is
-- raised OUTSIDE the expected-denial handler and must turn the harness red.
BEGIN;
...
DECLARE
  ...
  req_xfer uuid := '16161616-1616-4161-8161-161616161614';
  req_fixture uuid;
  ...
  step_cas uuid := '17171717-1717-4171-8171-171717171716';

  corr uuid := '18181818-1818-4181-8181-181818181811';
  corr2 uuid := '18181818-1818-4181-8181-181818181812';
  corr_cas uuid := '18181818-1818-4181-8181-181818181813';
  corr_bad uuid := '18181818-1818-4181-8181-181818181899';
  ...
  v_vis_before bigint;
  v_vis_after bigint;
  v_assignee_before uuid;
  v_assignee_after uuid;
...
BEGIN
  ...
  INSERT INTO public.departments(id,name_ar) VALUES (d_src,'مصدر'),(d_tgt,'هدف');
  ...
  INSERT INTO public.position_assignments(id,user_id,is_active,assigned_from) VALUES
    (pa_src,u_dh_src,true,CURRENT_DATE),(pa_tgt,u_dh_tgt,true,CURRENT_DATE);
  ...
  INSERT INTO public.request_processing_assignments(unit_id,role_id,assignment_type,user_id,is_active)
  SELECT unit_sa, role_sa, 'user', u_real_sa, true FROM generate_series(1,10);
  ...
  VALUES (wf_susp,cfg_review,NULL,'reviewed'),
  ...
  INSERT INTO public.student_request_workflow_steps(
    ...
    jsonb_build_object('direct_assignment_id', (SELECT id FROM public.request_processing_assignments WHERE user_id=u_real_sa LIMIT 1))
  );
  ...
  ------------------------------------------------------------------
  -- C) Creation while hidden — robust denial proofs
  ------------------------------------------------------------------
  PERFORM set_config('e_rpcmatrix.uid', u_student::text, true);
  v_denied := false;
  BEGIN
    PERFORM public.create_student_request('enrollment_suspension','denied','{}'::jsonb,NULL);
  EXCEPTION
    WHEN SQLSTATE '42501' THEN
      IF SQLERRM LIKE '%غير متاح للطالب%' OR SQLERRM LIKE 'B1_E2E_88%' THEN
        v_denied := true;
      ELSE
        RAISE;
      END IF;
  ...
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_HIDDEN_CREATE';
  END IF;
  ...
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_ENROLLMENT_CERT';
  END IF;
  ...
  IF public.b1_e2e_88_request_correlation(created_id) IS DISTINCT FROM corr THEN
    RAISE EXCEPTION 'C_REQUEST_CORRELATION_NOT_STAMPED';
  END IF;
  IF (SELECT form_data->>'e2e_immutable' FROM public.student_requests WHERE id=created_id)
     IS DISTINCT FROM 'true' THEN
    RAISE EXCEPTION 'C_MARKER_NOT_IMMUTABLE';
  END IF;

  -- missing / malformed correlation
  PERFORM public.open_b1_e2e_88_execution(
    '18181818-1818-4181-8181-181818181821'::uuid, u_student, 'enrollment_suspension', now() + interval '1 hour'
  );
  v_denied := false;
  BEGIN
    PERFORM public.create_student_request(
      'enrollment_suspension','missing corr',
      jsonb_build_object('e2e_marker','TEST_ONLY_B1_E2E_88'),
      NULL
    );
  EXCEPTION WHEN SQLSTATE '42501' THEN v_denied := true;
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_MISSING_CORR';
  END IF;

  v_denied := false;
  BEGIN
    PERFORM public.create_student_request(
      'enrollment_suspension','bad corr',
      jsonb_build_object('e2e_marker','TEST_ONLY_B1_E2E_88','e2e_correlation_id','not-a-uuid'),
      NULL
    );
  EXCEPTION WHEN SQLSTATE '42501' THEN v_denied := true;
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_MALFORMED_CORR';
  END IF;
  ...
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_OTHER_STUDENT';
  END IF;
  ...
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:C_SECOND_REQUEST';
  END IF;
  ...
  ------------------------------------------------------------------
  -- B) Request-scoped E2E authorization + correlation closure
  ------------------------------------------------------------------
  ...
  INSERT INTO public.student_request_workflow_steps(
    ...
    jsonb_build_object('direct_assignment_id','00000000-0000-4000-8000-000000000001')
  );
  INSERT INTO public.student_requests(id,request_number,student_profile_id,request_type,title,status,form_data)
  VALUES (
  ...
  -- Immutable correlation: student/generic form persistence cannot rewrite it.
  v_denied := false;
  BEGIN
    UPDATE public.student_requests
    SET form_data = form_data || jsonb_build_object('e2e_correlation_id', corr_bad::text)
    WHERE id = created_id;
  EXCEPTION
    WHEN SQLSTATE '42501' THEN
      IF SQLERRM LIKE '%B1_E2E_88_MARKER_IMMUTABLE%' THEN
        v_denied := true;
      ELSE
        RAISE;
      END IF;
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:IMMUTABLE_CORR_REWRITE';
  END IF;

  -- Department heads
  INSERT INTO public.student_requests(id,request_number,student_profile_id,request_type,title,status,form_data)
  VALUES (
    req_xfer,'SR-E2E-88-XFER',sp_student,'department_transfer','xfer','submitted',
    jsonb_build_object(
      'e2e_marker','TEST_ONLY_B1_E2E_88',
      'e2e_correlation_id',corr2::text,
      'e2e_immutable',true
  ...
  VALUES (req_xfer,d_src,d_tgt);
  PERFORM set_config('b1.atomic_init','1',true);
  INSERT INTO public.student_request_workflow_steps(
    id,student_request_id,workflow_id,workflow_step_id,step_key,step_order,status,
    processing_unit_id,processing_role_id,assigned_position_assignment_id,metadata
  ) VALUES
    (step_src,req_xfer,wf_xfer,cfg_src,'source_department_head_approval',1,'active',unit_dept,role_dh,pa_src,'{}'::jsonb),
    (step_tgt,req_xfer,wf_xfer,cfg_tgt,'target_department_head_approval',2,'pending',unit_dept,role_dh,pa_tgt,'{}'::jsonb);
  PERFORM set_config('b1.atomic_init','',true);

  PERFORM public.open_b1_e2e_88_execution(corr2, u_student, 'department_transfer', now() + interval '1 hour');
  UPDATE public.b1_e2e_88_executions SET created_request_id = req_xfer WHERE correlation_id = corr2;

  PERFORM public.bind_b1_e2e_88_actor_to_runtime_step(
    corr2, req_xfer, step_src, u_dh_src, 'approve', d_src, 'source'
  );
  ...
  PERFORM public.bind_b1_e2e_88_actor_to_runtime_step(
    corr2, req_xfer, step_tgt, u_dh_tgt, 'approve', d_tgt, 'target'
  );
  ...
  SET starts_at = now() - interval '2 hours', expires_at = now() - interval '1 second'
  ...
  END IF;
  UPDATE public.b1_e2e_88_executions
  SET starts_at = now() - interval '1 minute', expires_at = now() + interval '1 hour'
  WHERE correlation_id = corr;
  ...
  -- wrong correlation / non-TEST_ONLY / fixture bind denials
  v_denied := false;
  BEGIN
    PERFORM public.bind_b1_e2e_88_actor_to_runtime_step(
      corr_bad, created_id, step_e2e, u_e2e_sa, 'review'
    );
  EXCEPTION
    WHEN SQLSTATE '42501' THEN
      IF SQLERRM LIKE '%B1_E2E_88%' THEN v_denied := true; ELSE RAISE; END IF;
    WHEN SQLSTATE 'P0002' THEN v_denied := true;
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:B_WRONG_CORR';
  END IF;
  ...
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:B_NON_TEST_ONLY';
  END IF;
  ...
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:B_FIXTURE_BIND';
  END IF;
  ...
  ------------------------------------------------------------------
  -- D) Cleanup CAS success + drift failure
  ------------------------------------------------------------------
  -- Fresh CAS success path
  PERFORM set_config('e_rpcmatrix.uid', u_student::text, true);
  PERFORM public.open_b1_e2e_88_execution(corr_cas, u_student, 'enrollment_suspension', now() + interval '1 hour');
  cas_req := public.create_student_request(
    'enrollment_suspension','cas',
    jsonb_build_object('e2e_marker','TEST_ONLY_B1_E2E_88','e2e_correlation_id',corr_cas::text),
    NULL
  );
  UPDATE public.student_requests SET status='submitted' WHERE id=cas_req;
  PERFORM set_config('b1.atomic_init','1',true);
  INSERT INTO public.student_request_workflow_steps(
    id,student_request_id,workflow_id,workflow_step_id,step_key,step_order,status,
    processing_unit_id,processing_role_id,assigned_user_id,metadata
  ) VALUES (
    step_cas,cas_req,wf_susp,cfg_review,'initial_review',1,'active',unit_sa,role_sa,u_real_sa,
    jsonb_build_object('direct_assignment_id','00000000-0000-4000-8000-000000000099')
  );
  PERFORM set_config('b1.atomic_init','',true);
  PERFORM public.bind_b1_e2e_88_actor_to_runtime_step(
    corr_cas, cas_req, step_cas, u_e2e_sa, 'review'
  );

  -- Drift: simulate legitimate later reassignment, cleanup must fail and preserve it
  SELECT assigned_user_id INTO v_assignee_before
  FROM public.student_request_workflow_steps WHERE id=step_cas;
  SELECT count(*) INTO v_active_before FROM public.b1_e2e_88_actor_bindings
  WHERE correlation_id=corr_cas AND active;
  PERFORM set_config('b1.atomic_action','1',true);
  UPDATE public.student_request_workflow_steps
  SET assigned_user_id = u_real_sa, updated_at = now()
  WHERE id = step_cas;
  PERFORM set_config('b1.atomic_action','',true);

  v_denied := false;
  BEGIN
    PERFORM public.cleanup_b1_e2e_88_package(corr_cas, true);
  EXCEPTION
    WHEN SQLSTATE '42501' THEN
      IF SQLERRM LIKE '%B1_E2E_88_CLEANUP_ASSIGNEE_DRIFT%' THEN
        v_denied := true;
      ELSE
        RAISE;
      END IF;
  END;
  IF NOT v_denied THEN
    RAISE EXCEPTION 'B1_E2E_88_HARNESS_UNEXPECTED_SUCCESS:D_CAS_DRIFT';
  END IF;

  SELECT assigned_user_id INTO v_assignee_after
  FROM public.student_request_workflow_steps WHERE id=step_cas;
  IF v_assignee_after IS DISTINCT FROM u_real_sa THEN
    RAISE EXCEPTION 'D_CAS_LATER_ASSIGNMENT_NOT_PRESERVED';
  END IF;
  SELECT count(*) INTO v_active_after FROM public.b1_e2e_88_actor_bindings
  WHERE correlation_id=corr_cas AND active;
  IF v_active_after IS DISTINCT FROM v_active_before THEN
    RAISE EXCEPTION 'D_CAS_PARTIAL_DEACTIVATE_FORBIDDEN';
  END IF;

  -- Restore E2E applied state and cleanup successfully
  PERFORM set_config('b1.atomic_action','1',true);
  UPDATE public.student_request_workflow_steps
  SET assigned_user_id = u_e2e_sa,
  ...
  PERFORM set_config('e_rpcmatrix.uid', u_e2e_sa::text, true);
  IF public.can_current_user_act_on_step(step_cas,'review') THEN
    RAISE EXCEPTION 'D_BINDINGS_SHOULD_BE_UNUSABLE';
  END IF;

  -- Close remaining open executions via cleanup (corr already closed; corr2 open)
