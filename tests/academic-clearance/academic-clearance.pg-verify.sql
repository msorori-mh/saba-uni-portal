...
insert into academic_clearance_cases(id,student_request_id,student_profile_id,source_department_id,target_department_id,target_study_plan_id,status,source_snapshot_at,target_snapshot_at,remaining_credit_hours) values('d0000000-0000-4000-8000-000000000001','80000000-0000-4000-8000-000000000001','70000000-0000-4000-8000-000000000001','20000000-0000-4000-8000-000000000001','20000000-0000-4000-8000-000000000002','50000000-0000-4000-8000-000000000001','draft',now(),now(),6);
...
do $$begin begin perform save_academic_clearance_equivalency('d0000000-0000-4000-8000-000000000001',1,'d1000000-0000-4000-8000-000000000001','d2000000-0000-4000-8000-000000000001','equivalent',3,'wrong actor'); raise exception 'expected chair denial'; exception when others then if sqlerrm not like '%ACADEMIC_CLEARANCE_FORBIDDEN%' then raise; end if; end; end$$;
set role authenticated;
do $$begin if (select count(*) from academic_clearance_cases)<>0 then raise exception 'RLS wrong actor leak'; end if; end$$;
reset role;
select set_config('request.jwt.claim.sub','10000000-0000-4000-8000-000000000001',false);
set role authenticated;
do $$begin if (select count(*) from academic_clearance_cases)<>1 then raise exception 'RLS direct chair allow failed'; end if; end$$;
reset role;
select save_academic_clearance_equivalency('d0000000-0000-4000-8000-000000000001',1,'d1000000-0000-4000-8000-000000000001','d2000000-0000-4000-8000-000000000001','equivalent',3,'matched');
select submit_academic_clearance_for_review('d0000000-0000-4000-8000-000000000001',2,'chair approved comparison');
do $$begin
 begin perform approve_academic_clearance('d0000000-0000-4000-8000-000000000001',3,'x'); raise exception 'expected wrong actor denial'; exception when others then if sqlerrm not like '%ACADEMIC_CLEARANCE_FORBIDDEN%' then raise; end if; end;
...
select approve_academic_clearance('d0000000-0000-4000-8000-000000000001',3,'academic approval');
do $$begin
...
update student_requests set status='approved' where id='80000000-0000-4000-8000-000000000001';
select correct_academic_clearance('d0000000-0000-4000-8000-000000000001',4,'documented correction');
