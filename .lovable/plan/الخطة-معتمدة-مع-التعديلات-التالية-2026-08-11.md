# الخطة معتمدة مع التعديلات التالية:

1. فصل التحقق:

   validateDraftPolicy

   - يسمح بالحقول الأكاديمية الفارغة.

   - يمنع فقط القيم البنيوية غير الصحيحة.

   - SAVE DRAFT = ALLOW عند النقص.

   validatePolicyForPublish

   - تحقق UX محلي فقط.

   - المصدر الحاكم قبل النشر هو gp_admin_validate_policy في الـBackend.

   - ثم gp_admin_publish_policy.

2. لا تجعل proposal_window وdefense_window إلزاميتين دائمًا.

أضف قرارًا صريحًا للإدارة لكل نافذة:
...
يمكن أن يكون الاختيار غير محدد.

Publish:

يجب أن يختار Admin صراحة true أو false.

إذا enforce_proposal_window=true:

proposal_window_start/end مطلوبان والبداية <= النهاية.

إذا enforce_proposal_window=false:

لا تفرض نافذة زمنية، ويجب ألا تؤثر قيم تاريخ قديمة على Runtime.

ونفس العقد لـ defense window.

الهدف:

NO_WINDOW ليس خطأ؛ هو سياسة إدارية صحيحة عندما تختارها الإدارة صراحة.

3. لا تنشر Policy TEST_ONLY في Global scope.

أي Production E2E للسياسة يجب أن يكون في Scope TEST_ONLY معزولًا لا يطابق أي مشروع حقيقي.

لا DELETE لـ Published Policy للتحايل على immutability.

استخدم cleanup/supersession governed أو disposable rehearsal.

العقد النهائي:

INCOMPLETE_DRAFT_SAVE = ALLOW

STRUCTURALLY_INVALID_DRAFT_SAVE = DENY

INCOMPLETE_POLICY_PUBLISH = DENY

COMPLETE_VALID_POLICY_PUBLISH = ALLOW

PROPOSAL_WINDOW_MODE = ADMIN_CONFIGURABLE

DEFENSE_WINDOW_MODE = ADMIN_CONFIGURABLE

PUBLISHED_POLICY_IMMUTABLE = PASS

BACKEND_VALIDATION_AUTHORITATIVE = PASSGP-3 — تصحيح عقد المسودة/النشر وإغلاق الملف
