# الخطة **معتمدة للتنفيذ**، وهي بالضبط الإغلاق المحدود الذي نحتاجه قبل تصريح الإنتاج.

```text
APPROVED_SOURCE_REMEDIATION_MOBILE_STEP_UP_TRUST_ENROLLMENT_01

```

لا أريد توسيع النطاق. نفّذ البنود الخمسة كما هي، مع تشديدين صغيرين في الإثبات:

- فحص الصلاحيات يكون فعليًا عبر `has_function_privilege(...)` ولقطة `pg_proc.proacl` بعد تطبيق الـMigration على PostgreSQL الاختباري، وليس فحص نص SQL فقط.
- أثبت أيضًا أن `PUBLIC` و`anon` **لا يملكان EXECUTE** على `register_student_device` و`issue_step_up_challenge`، وأن `authenticated` يحتفظ فقط بدوال الإبطال المطلوبة.

الـcontract النهائي المتوقع:
...
وأريد نتيجة الاختبار النهائية بهذه الصورة:
...
إذا خرجت النتيجة بهذه الصورة ولم يظهر blocker جديد **داخل هذا النطاق نفسه**، فلن نفتح جولة تصميم جديدة؛ الخطوة التالية مباشرة ستكون إصدار:

`APPROVED_PRODUCTION_APPLY_MOBILE_STEP_UP_01`

ثم التطبيق الإنتاجي المحدود، وبعده Android physical test وبوابة Go-Live.إغلاق فجوة تسجيل الثقة (Trust Enrollment) — مصدر فقط
