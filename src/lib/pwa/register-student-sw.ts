/**
 * Registers the Student Mobile PWA service worker.
 ...
 * - Only attempted when the browser supports service workers and is online-capable.
 *
 * Safe to call multiple times — `register()` is idempotent per scope.
 */
export function registerStudentMobileSW(): void {
  ...
  const run = () => {
    navigator.serviceWorker
      ...
      .catch((err) => {
        ...
        console.warn("[student-pwa] SW registration failed:", err);
      });
  };
