...
type RoleRef = { role_code: string; source_type: string; position_name: string | null };
    ...
    <div className="space-y-6" dir="rtl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          الإسناد المباشر والاستثنائي للأدوار. الأدوار المشتقة من المناصب تُدار من صفحة الهيكل التنظيمي ولا تُحذف من هنا.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card shadow-card overflow-x-auto">
        {usersQ.isLoading ? (
          ...
          <div className="p-8 text-center space-y-3">
            <div className="text-destructive text-sm">تعذّر تحميل المستخدمين: {(usersQ.error as Error).message}</div>
            <button onClick={() => usersQ.refetch()} className="inline-flex items-center gap-1 text-sm px-3 py-1.5 rounded border hover:bg-muted">
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-right">
              <tr>
                ...
                <th className="p-3">الصلاحيات التشغيلية</th>
            ...
            <tbody>
                ...
                <tr key={u.user_id} className="border-t align-top">
                  ...
                  <td className="p-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(u.roles as RoleRef[]).map((ref) => {
                        const rc = ref.role_code;
                        const r = (roles ?? []).find((x: any) => x.code === rc);
                        const derived = ref.source_type === "position";
                        return (
                          <span
                            key={rc}
                            className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded ${derived ? "bg-muted text-foreground" : "bg-primary/10 text-primary"}`}
                            title={derived ? `مشتق من منصب: ${ref.position_name ?? ""}` : "إسناد مباشر"}
                          >
                            {r?.name_ar ?? rc}
                            {derived ? (
                              <span className="text-[10px] px-1 rounded bg-sky-100 text-sky-700 inline-flex items-center gap-0.5">
                                <Lock className="h-2.5 w-2.5" /> من منصب: {ref.position_name ?? "—"}
                              </span>
                            ) : (
                              <button
                  ...
                  <td className="p-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(u.app_roles ?? []).map((ar: string) => (
                        <span key={ar} className="text-[11px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-mono">{ar}</span>
                      ))}
                    </div>
                  </td>
                  <td className="p-3">
                    <button
                      onClick={() => setPicker({
                        user_id: u.user_id,
                        name: u.name || u.email || "",
                        roles: (u.roles as RoleRef[]).map((r) => r.role_code),
                      })}
              ...
              {rows.length === 0 && (
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">لا توجد نتائج مطابقة للبحث</td></tr>
              )}
            </tbody>
          </table>
        )}
