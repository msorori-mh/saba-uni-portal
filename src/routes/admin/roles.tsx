          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-right">
              <tr>
                ...
                <th className="p-3">الدور التشغيلي</th>
            ...
            <tbody>
              {(rows ?? []).map((r: any) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="p-3">
                    {r.app_role_mapping ? (
                      <span className="text-xs px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 font-mono">{r.app_role_mapping}</span>
                    ) : (
                      <span className="text-xs px-2 py-0.5 rounded bg-amber-100 text-amber-700">وصفي فقط</span>
                    )}
                  </td>
