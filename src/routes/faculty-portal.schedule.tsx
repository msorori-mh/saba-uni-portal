...
import { CalendarClock, Loader2, Printer, FileSpreadsheet } from "lucide-react";
...
async function fetchData(): Promise<{ rows: ScheduleRow[]; info: FacultyInfo | null }> {
  ...
  const info: FacultyInfo = {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- pre-existing untyped join; guarded by tests/student-portal/current-term-schedule-filters-01.test.ts
...
function FacultySchedulePage() {
  ...
  return (
    <FacultyPortalShell
      ...
      breadcrumbs={[{ label: "جدول التدريس" }]}
      hideChromeOnPrint
    >
      <div className="container mx-auto px-4 py-6 max-w-5xl space-y-5 print-page">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 no-print">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" onClick={handlePrint} disabled={!rows.length}>
              <Printer className="h-4 w-4 ms-1" aria-hidden /> طباعة
            </Button>
            <Button size="sm" variant="outline" onClick={handleExport} disabled={!rows.length}>
              <FileSpreadsheet className="h-4 w-4 ms-1" aria-hidden /> تصدير Excel
            </Button>
          </div>
        </div>
