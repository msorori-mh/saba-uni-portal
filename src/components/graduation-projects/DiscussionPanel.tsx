...
export function DiscussionPanel({
  ...
  const scheduledDiscussions = discussions.filter((discussion) =>
    ["scheduled", "postponed"].includes(discussion.state),
  );
    ...
    <div dir="rtl" className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {assessment.ready ? "المشروع جاهز لطلب المناقشة." : "هناك متطلبات غير مكتملة."}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          {assessment.blockers.map((blocker) => (
            <p key={blocker} className="text-sm text-destructive">
              {READINESS_BLOCKER_LABELS[blocker] ?? blocker}
            </p>
          ))}
          {assessment.atRisk ? <Badge variant="destructive">المشروع متعثر</Badge> : null}
          {canRequest ? (
            <Button type="button" disabled={busy} onClick={onRequestDiscussion}>
              طلب مناقشة
            </Button>
          ) : null}
        </CardContent>
      ...
      {pendingRequests.length > 0 && (canSchedule || canReject) ? (
        <Card>
          <CardHeader>
            <CardTitle>طلبات المناقشة المعلقة</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {pendingRequests.map((request) => (
              <div key={request.id} className="space-y-2 border-b pb-3">
                <p className="text-sm">طلب بتاريخ {formatGpDateTimeAr(request.requested_at)}</p>
                {canSchedule ? (
                  <div className="grid gap-2">
                    <Label htmlFor={`gp-schedule-at-${request.id}`}>موعد المناقشة</Label>
                    <Input
                      id={`gp-schedule-at-${request.id}`}
                      value={startsAt}
                      ...
                      dir="ltr"
                    />
                    <Label htmlFor={`gp-schedule-venue-${request.id}`}>المكان</Label>
                    <Input
                      id={`gp-schedule-venue-${request.id}`}
                      value={venue}
                      onChange={(event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
                        setVenue(event.target.value)
                      }
                      placeholder="قاعة المناقشات"
                    />
                    <Button
                      type="button"
                      disabled={busy || startsAt.trim() === "" || venue.trim() === ""}
      ...
      {scheduledDiscussions.length > 0 ? (
        <Card>
          <CardHeader>
            <CardTitle>المناقشات المجدولة</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {scheduledDiscussions.map((discussion) => (
              <div key={discussion.id} className="space-y-2 border-b pb-3">
                <p className="text-sm">
                  {formatGpDateTimeAr(discussion.starts_at)} — {discussion.venue} —{" "}
                  {DISCUSSION_STATE_LABELS[discussion.state] ?? discussion.state}
                </p>
                <p className="text-sm text-muted-foreground">
                  أعضاء اللجنة:{" "}
                  {panelMembers.filter((member) => member.discussion_id === discussion.id).length}
                </p>
                {canAssignPanel && discussion.state === "scheduled" ? (
                  <div className="flex flex-wrap items-center gap-2">
                    <select
                      value={selectedPanelAssignment}
                      onChange={(event) => setSelectedPanelAssignment(event.target.value)}
                      aria-label="اختيار تعيين عضو اللجنة"
                      className="min-h-11 rounded-lg border border-border bg-background px-3 text-sm"
                    >
                      <option value="">اختر عضو اللجنة…</option>
                      {panelCandidates.map((candidate, index) => (
                        <option key={candidate.id} value={candidate.id}>
                          عضو لجنة محتمل {index + 1}
                        </option>
                      ))}
                    </select>
                    <Button
                      type="button"
                      variant="secondary"
                      disabled={busy || selectedPanelAssignment.trim() === ""}
                      onClick={() =>
                        onAssignPanelMember(discussion.id, selectedPanelAssignment.trim(), false)
                      }
                    >
                      تعيين عضو لجنة
                    </Button>
                ...
                {canRecord ? (
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      disabled={busy}
                      onClick={() => onRecordOutcome(discussion.id, "held")}
                    >
                      انعقدت
                    </Button>
                    <Button
                      ...
                      variant="secondary"
                      disabled={busy}
                      onClick={() => onRecordOutcome(discussion.id, "postponed")}
                    >
                      أُجّلت
                    </Button>
                    <Button
                      ...
                      variant="destructive"
                      disabled={busy}
                      onClick={() => onRecordOutcome(discussion.id, "cancelled")}
                    >
                      أُلغيت
                    </Button>
                  </div>
            ...
            {panelCandidates.length > 0 ? (
              <p className="text-sm text-muted-foreground">
                تعيينات اللجنة المتاحة: {panelCandidates.length}
              </p>
            ) : null}
          </CardContent>
        </Card>
