...
export type ForgotPasswordCtx = "admin" | "student" | "faculty" | "staff";
type Ctx = ForgotPasswordCtx;

/** Surfaces allowed as a "back" target for the recovery screen. */
export type ForgotPasswordBackTo = "/admin/login" | "/portal-login" | "/mobile/student-login";
        ...
        <div className="rounded-2xl border-2 border-gold/40 bg-card shadow-elegant overflow-hidden">
          ...
          {done ? (
            <div className="px-8 py-10 text-center space-y-4">
              ...
              <div className="pt-4">
                <Link to={backToOverride ?? cfg.backTo} className="text-sm font-bold text-primary hover:text-gold">
                  {backLabelOverride ?? cfg.backLabel}
                </Link>
              </div>
            </div>
          ) : sent ? (
            <form onSubmit={onVerify} className="px-8 py-8 space-y-5">
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                ...
                <Link to={backToOverride ?? cfg.backTo} className="text-primary hover:text-gold font-bold inline-flex items-center gap-1">
                  <ArrowRight className="h-3 w-3" /> {backLabelOverride ?? cfg.backLabel}
                </Link>
              </div>
            </form>
