-- =====================================================================
-- ACADEMIC-COUNCILS-C5-MINUTES-LIFECYCLE-02
-- PROMOTED MIGRATION - NOT APPLIED TO PRODUCTION
-- REQUIRES EXPLICIT SINGLE-MIGRATION APPROVAL
-- CANONICAL_APPLY_CANDIDATE for logical C5
--
-- Source revision of 20260808150000_councils_c5_minutes_lifecycle_01
-- (SUPERSEDED_DO_NOT_APPLY; never applied to production).
--
-- Minimum correction vs V1:
--   - Fail-closed prerequisite: extensions.digest(text,text) must exist
--   - Fully qualify fingerprint hash as extensions.digest(...) under
--     SECURITY DEFINER search_path = public, pg_temp
--   Do NOT CREATE EXTENSION here; do NOT add extensions to search_path.
--
-- Scope:
...
-- ---------------------------------------------------------------------
-- 0) Guard: C4 session & voting must exist; extensions.digest prerequisite
-- ---------------------------------------------------------------------
DO $$
BEGIN
  ...
  END IF;
  -- Fail-closed: pgcrypto digest must already live in schema extensions (production contract).
  -- Do not CREATE EXTENSION in this migration.
  IF to_regprocedure('extensions.digest(text,text)') IS NULL THEN
    RAISE EXCEPTION 'C5 minutes lifecycle requires extensions.digest(text,text); CREATE EXTENSION is forbidden in this migration';
  END IF;
END $$;
...
BEGIN
  ...
  -- Compute deterministic SHA256 fingerprint (fully qualify: search_path excludes extensions)
