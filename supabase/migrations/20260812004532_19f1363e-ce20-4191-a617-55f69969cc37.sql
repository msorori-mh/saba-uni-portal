-- 1) Faculty-only execution confirmation (owner contract: faculty alone confirms)
CREATE OR REPLACE FUNCTION public.cdp_record_session_execution(p_plan_session_id uuid, p_status text, p_execution_date date, p_reason text, p_compensation_date date, p_notes text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.cdp_clear_session_execution(p_plan_session_id uuid)
 ...
 SET search_path TO 'public'
AS $function$
declare v_section uuid; v_uid uuid := auth.uid();
begin
  if v_uid is null then raise exception 'CDP_UNAUTHENTICATED'; end if;
  select p.course_section_id into v_section
  from public.course_delivery_plan_sessions s
...
-- 2) Scoped period monitoring for department head / academic affairs / dean / admin
...
begin
  ...
  create temporary table if not exists _cdp_scope_tmp () on commit drop;
...
REVOKE ALL ON FUNCTION public.cdp_delivery_monitoring(text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.cdp_delivery_monitoring(text) TO authenticated;