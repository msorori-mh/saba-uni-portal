...
-- Migration: 20260808210200_ga_authorization_04.sql
-- Source draft: docs/migration-drafts/GRADUATES-AFFAIRS-AUTHORIZATION-04.sql
-- HASH_CONTRACT: SHA256_LF_NORMALIZED_V1
-- Predecessor: 20260808210100_ga_mvp_completion_01.sql
-- DO NOT APPLY until production read-only preflight PASSes and Foundation + Completion are applied.
--
-- Semantics: Graduates affairs authorization bundle 04: actor-model resolution helpers,
...
do $$ begin
  -- Foundation + Completion must be present
  if to_regclass('public.graduate_records') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: graduate_records (foundation) required';
  end if;
  if to_regclass('public.graduate_followups') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: graduate_followups (completion) required';
  end if;
  if to_regclass('public.graduate_communication_events') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: graduate_communication_events (completion) required';
  end if;
  if to_regclass('public.graduate_account_continuity_policies') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: graduate_account_continuity_policies (completion) required';
  end if;

  -- Assignment infrastructure must be present
  if to_regclass('public.request_processing_assignments') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: request_processing_assignments required';
  end if;
  if to_regclass('public.staff_profile_departments') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: staff_profile_departments required';
  end if;
  if to_regclass('public.staff_profiles') is null then
    raise exception 'GA_AUTH04_PREFLIGHT_MISSING: staff_profiles required';
  end if;

  -- Re-apply guard: detect already-present authorization surface
  if exists (
    ...
    raise exception 'GA_AUTH04_PREFLIGHT_ALREADY_APPLIED: GA RLS policies already present';
  end if;
end $$;
