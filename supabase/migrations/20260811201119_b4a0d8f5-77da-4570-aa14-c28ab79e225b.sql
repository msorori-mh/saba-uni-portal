...
BEGIN
  ...
  -- Historical enrollment rows are incomplete for legacy students, so the
  -- current academic level is a trusted lower bound: level 2 implies one
  -- completed academic year, level 3 implies two, and so on.
