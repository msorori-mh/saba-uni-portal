-- CAPACITY_04: a filtered student transcript must not aggregate unrelated grades.
-- Preserve all output columns, grade rules, appeal precedence and equivalencies.
-- CREATE OR REPLACE preserves ownership/grants/dependent views; keep invoker RLS.
           ...
           FROM student_enrollments se
             JOIN LATERAL (
               SELECT sg.student_enrollment_id,
                  COALESCE((
                    SELECT gad.approved_final_result
                    FROM public.grade_appeal_details gad
                    WHERE gad.student_enrollment_id = se.id
                      AND gad.result_change_applied_at IS NOT NULL
               ...
               WHERE sg.student_enrollment_id = se.id AND sg.status = 'approved'
