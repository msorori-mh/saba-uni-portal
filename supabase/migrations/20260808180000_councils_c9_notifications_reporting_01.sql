...
AS $$
DECLARE
  v_auth uuid := public.council_attendance_require_auth_uid();
  -- Ignore client-supplied user id (IDOR). Always scope to authenticated caller.
  v_uid uuid := v_auth;
BEGIN
  IF p_user_id IS NOT NULL AND p_user_id IS DISTINCT FROM v_auth THEN
    RAISE EXCEPTION 'COUNCIL_RESPONSIBLE_DECISIONS_IMPERSONATION_DENIED' USING ERRCODE = '42501';
  END IF;
