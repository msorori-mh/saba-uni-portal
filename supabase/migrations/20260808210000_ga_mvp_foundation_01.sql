...
-- Package: PORTAL-GRADUATES-AFFAIRS-PRODUCTION-PROMOTION-LONGRUN-09
-- Migration: 20260808210000_ga_mvp_foundation_01.sql
-- Source draft: docs/migration-drafts/GRADUATES-AFFAIRS-MVP-FOUNDATION-01.sql
-- HASH_CONTRACT: SHA256_LF_NORMALIZED_V1
-- Production scenario: graduate_affairs unit and roles already seeded by 20260716172804_3baccc1f-0879-4a6c-ad1b-225616d024c8.sql
-- Predecessor chain (canonical): upstream student/staff/department schema -> this foundation
-- DO NOT APPLY until production read-only preflight PASSes.
--
-- Semantics: Graduates affairs MVP foundation. No production activation or data seed.
...
do $$ begin
  -- Fail-closed dependency guards
  if to_regclass('public.student_profiles') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: student_profiles required';
  end if;
  if to_regclass('public.staff_profiles') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: staff_profiles required';
  end if;
  if to_regclass('public.departments') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: departments required';
  end if;
  if to_regclass('public.programs') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: programs required';
  end if;
  if to_regclass('public.staff_profile_departments') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: staff_profile_departments required';
  end if;
  if to_regclass('public.request_processing_units') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: request_processing_units required';
  end if;
  if to_regclass('public.request_processing_roles') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: request_processing_roles required';
  end if;
  if to_regclass('public.request_processing_assignments') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: request_processing_assignments required';
  end if;
  if to_regclass('auth.users') is null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING: auth.users required';
  end if;

  -- Canonical unit/role prestate reconciliation (idempotent read-only check)
  if not exists (
    select 1 from public.request_processing_units
    where code = 'graduate_affairs' and is_active
  ) then
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING_UNIT: graduate_affairs unit must exist and be active';
  end if;
  ...
  if (
    ...
    raise exception 'GA_FOUNDATION_PREFLIGHT_MISSING_ROLES: graduate_affairs_manager and graduate_affairs_specialist must exist and be active';
  end if;

  -- Partial-apply / re-apply guard: foundation objects must not already exist
  if to_regclass('public.graduate_records') is not null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_ALREADY_APPLIED: graduate_records already exists';
  end if;
  if to_regclass('public.graduate_official_decisions') is not null then
    raise exception 'GA_FOUNDATION_PREFLIGHT_ALREADY_APPLIED: graduate_official_decisions already exists';
  end if;
end $$;
