...
--
-- NOTE: File intentionally located under docs/drafts/ (NOT under
-- supabase/migrations/) so no automated tooling can apply it. When the
-- human review is complete, the next phase (COUNCILS-MVP-SCAFFOLD-01)
-- will submit this SQL through the migration tool for approval.
--
-- Rules honored:
--   * No ALTER/DROP/TRUNCATE against existing tables.
--   * No seed data. No storage bucket. No notifications. No email.
--   * No schedule rules. No attachments. No followups table.
--   * DELETE is REVOKED on every new table (soft-archive only).
--   * All policies use SECURITY DEFINER helpers — no self-recursion.
--   * No GRANT to anon. service_role kept full for admin ops.
-- =====================================================================
...
-- =====================================================================
-- 1. CORE TABLE — academic_councils
-- =====================================================================
...
-- =====================================================================
-- 6. academic_council_minutes  (one row per meeting)
-- =====================================================================
...
-- =====================================================================
-- 8. HELPER FUNCTIONS (SECURITY DEFINER, STABLE)
--    All isolate councils logic and avoid RLS self-recursion.
-- =====================================================================
...
AS $$
  SELECT
    public.has_role(_user, 'system_admin'::public.app_role)
...
-- =====================================================================
-- 9. RLS POLICIES  (see report §5 for the full matrix)
-- =====================================================================

-- 9.1 academic_councils
CREATE POLICY "councils_select"
  ON public.academic_councils FOR SELECT TO authenticated
  USING (public.is_council_admin(auth.uid()) OR public.is_council_member(auth.uid(), id));

CREATE POLICY "councils_insert_admin"
  ON public.academic_councils FOR INSERT TO authenticated
  WITH CHECK (public.is_council_admin(auth.uid()) AND created_by = auth.uid());

CREATE POLICY "councils_update_admin_or_chair"
  ON public.academic_councils FOR UPDATE TO authenticated
  USING (public.can_manage_council(auth.uid(), id))
  WITH CHECK (public.can_manage_council(auth.uid(), id));

-- 9.2 academic_council_members
CREATE POLICY "council_members_select"
  ON public.academic_council_members FOR SELECT TO authenticated
  USING (public.is_council_admin(auth.uid()) OR public.is_council_member(auth.uid(), council_id));

CREATE POLICY "council_members_insert"
  ON public.academic_council_members FOR INSERT TO authenticated
  WITH CHECK (public.can_manage_council(auth.uid(), council_id) AND created_by = auth.uid());

CREATE POLICY "council_members_update"
  ON public.academic_council_members FOR UPDATE TO authenticated
  USING (public.can_manage_council(auth.uid(), council_id))
  WITH CHECK (public.can_manage_council(auth.uid(), council_id));

-- 9.3 academic_council_meetings
CREATE POLICY "meetings_select"
  ON public.academic_council_meetings FOR SELECT TO authenticated
  USING (public.is_council_admin(auth.uid()) OR public.is_council_member(auth.uid(), council_id));

CREATE POLICY "meetings_insert"
  ON public.academic_council_meetings FOR INSERT TO authenticated
  WITH CHECK (public.can_write_council_agenda(auth.uid(), council_id) AND created_by = auth.uid());

CREATE POLICY "meetings_update"
  ON public.academic_council_meetings FOR UPDATE TO authenticated
  USING (public.can_write_council_agenda(auth.uid(), council_id))
  WITH CHECK (public.can_write_council_agenda(auth.uid(), council_id));

-- 9.4 academic_council_topics
CREATE POLICY "topics_select"
  ON public.academic_council_topics FOR SELECT TO authenticated
  USING (
...
CREATE POLICY "topics_insert_member"
  ON public.academic_council_topics FOR INSERT TO authenticated
  WITH CHECK (
...
CREATE POLICY "topics_update_owner_draft"
  ON public.academic_council_topics FOR UPDATE TO authenticated
  USING (
...
-- 9.5 academic_council_agenda_items
CREATE POLICY "agenda_select"
  ON public.academic_council_agenda_items FOR SELECT TO authenticated
  USING (
    public.is_council_admin(auth.uid())
    OR EXISTS (
...
CREATE POLICY "agenda_insert"
  ON public.academic_council_agenda_items FOR INSERT TO authenticated
  WITH CHECK (
    created_by = auth.uid()
    AND EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
...
CREATE POLICY "agenda_update"
  ON public.academic_council_agenda_items FOR UPDATE TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
      WHERE mt.id = meeting_id
...
-- 9.6 academic_council_minutes
CREATE POLICY "minutes_select"
  ON public.academic_council_minutes FOR SELECT TO authenticated
  USING (
    public.is_council_admin(auth.uid())
    OR EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
...
CREATE POLICY "minutes_insert_secretary"
  ON public.academic_council_minutes FOR INSERT TO authenticated
  WITH CHECK (
    drafted_by = auth.uid()
    AND EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
      WHERE mt.id = meeting_id
        AND (
          public.is_council_admin(auth.uid())
          OR public.has_council_role(auth.uid(), mt.council_id,
                                     'secretary'::public.academic_council_member_role)
        )
    )
  );

CREATE POLICY "minutes_update_before_lock"
  ON public.academic_council_minutes FOR UPDATE TO authenticated
  USING (
    is_locked = false
    AND EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
...
-- 9.7 academic_council_decisions
CREATE POLICY "decisions_select"
  ON public.academic_council_decisions FOR SELECT TO authenticated
  USING (
    public.is_council_admin(auth.uid())
...
CREATE POLICY "decisions_insert"
  ON public.academic_council_decisions FOR INSERT TO authenticated
  WITH CHECK (
    created_by = auth.uid()
    AND EXISTS (
      SELECT 1 FROM public.academic_council_meetings mt
...
-- =====================================================================
-- 10. TRIGGERS — updated_at + minutes lock immutability + type guard
--     (Ordering / audit / archive triggers deferred to next migration.)
-- =====================================================================
...
-- Prevent modification of a locked minutes record (immutability guard).
...
-- Guard: council_type='department' ⇒ department_id NOT NULL, and vice versa.
...
-- =====================================================================
-- END OF DRAFT — no seed, no storage, no notifications.
-- Next phase (COUNCILS-MVP-SCAFFOLD-01) will apply this via the
-- migration tool after human review.
-- =====================================================================
