...
4. **إثبات بصمة الكود المنشور (Deployed SHA Proof Verification)**: إجراء قراءة عادية (Read-back) من البيئة المباشرة لتأكيد أن `DEPLOYED_SHA` يطابق `RC313_SHA` المعتمد ديناميكياً (والذي يتضمن PR #314).
