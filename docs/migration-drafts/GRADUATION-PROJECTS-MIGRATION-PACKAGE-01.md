...
| M9 | `docs/migration-drafts/GRADUATION-PROJECTS-M9-AUDIT-REMEDIATION-06.NOT_APPLIED.sql` (k3 sequence: `20260730100008`) | Audit-05 remediation: F-1 rank boundary on assignment termination, F-2 settings/rubric canonical audit events (department-scoped `graduation_project_events` extension), F-6/F-7/F-9 low fixes | preflight in-file (M1..M8 sentinels + ambiguous-retry guard) | `tests/graduation-projects/audit-06/` (F-1 rank matrix, F-2 audit/correlation matrix, low-finding regressions) + all prior verifiers re-run post-M9 |
...
- **M9**: +1 column (`graduation_project_events.department_id`, nullable, FK), `project_id` nullability dropped, +1 scope CHECK (`graduation_project_events_scope`), +1 partial unique index (`graduation_project_events_department_correlation_key`), +1 internal function (`graduation_project_assignment_rank`, no app grant), 6 `create or replace` (`end_graduation_project_assignment` rank boundary, `upsert_graduation_project_settings` + `upsert_graduation_project_rubric` audit events + correlation, `create_graduation_project` department-scoped replay, `add_graduation_project_team_member` replay-first, `resolve_graduation_project_supervisor_note` note ownership); signatures and grants unchanged; 0 data rows.
