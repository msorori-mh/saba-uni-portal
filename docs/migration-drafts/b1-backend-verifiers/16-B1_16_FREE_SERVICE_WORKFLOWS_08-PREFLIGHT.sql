-- ============================================================================
-- B1-BACKEND-IMPLEMENTATION-01 / order 16 / PREFLIGHT
...
WITH checks(check_name, detail, ok) AS (
  SELECT 'CHECK_01', 'to_regclass(''public.request_type_workflows'') IS NOT NULL', (to_regclass('public.request_type_workflows') IS NOT NULL)
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'B1_ORDER_16_PREFLIGHT_FAILED';
  END IF;
END
$guard$;
