-- READ ONLY
-- Structural subset (sections 0-4) of
--   29-B1_29_RUNTIME_ASSIGNEE_PROPAGATION_01-POST-VERIFIER.sql
--
-- Purpose: sections 0-4 are pure catalog/definition checks and hold on ANY
-- cluster where order 29 has been applied, including the disposable Postgres
-- 17 cluster used by tests/b1-runtime-assignee-lock-concurrency-01. Section 5
-- of the full post-verifier asserts PRODUCTION data invariants (exactly one
-- assignee per B1 runtime step, exactly one active step per open B1 request);
-- the synthetic concurrency fixtures deliberately violate those invariants,
-- so section 5 is NOT reproducible on the harness cluster and is NOT weakened
-- or removed here. Run the FULL post-verifier (sections 0-5) against
-- production, read-only, at apply time.
BEGIN;
DO $$
DECLARE
  v_def text;
