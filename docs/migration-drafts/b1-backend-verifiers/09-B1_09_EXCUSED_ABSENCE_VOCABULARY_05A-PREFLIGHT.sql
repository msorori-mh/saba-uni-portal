-- ============================================================================
-- B1-BACKEND-IMPLEMENTATION-01 / order 9 / PREFLIGHT
...
WITH checks(check_name, detail, ok) AS (
  SELECT 'CHECK_01', 'to_regclass(''public.absence_excuse_details'') IS NOT NULL', (to_regclass('public.absence_excuse_details') IS NOT NULL)
)
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'B1_ORDER_9_PREFLIGHT_FAILED';
  END IF;
END
$guard$;
