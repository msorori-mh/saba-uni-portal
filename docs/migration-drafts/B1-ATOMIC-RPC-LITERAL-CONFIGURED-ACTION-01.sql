...
-- B1-ATOMIC-RPC-LITERAL-CONFIGURED-ACTION-01
-- DRAFT / FORWARD-ONLY. Applied to the ISOLATED non-production cluster only.
-- DO NOT APPLY TO PRODUCTION WITHOUT A SEPARATE EXPLICIT AUTHORIZATION.
--
-- Finding (isolated matrix, class illegal_action_by_exact_assignee):
--   The exact direct assignee of a step configured as clear / apply_decision /
--   archive could execute the RPC by sending the generic action 'approve',
--   because the UI-compatibility mapping rewrote it into the configured action.
--   The B1 source contract requires p_action = configured action_type with no
--   generic fallback. This draft closes that gap.
...
BEGIN
  ...
  -- B1 LITERAL ACTION CONTRACT: the caller must send the configured action_type
  -- verbatim. The former UI compatibility translation
  -- (b1_map_ui_staff_action(action_type) = p_action) allowed a generic
  -- 'approve' to stand in for clear / apply_decision / archive and is removed.
  v_action := p_action;
  IF v_config.action_type IS NULL OR p_action IS DISTINCT FROM v_config.action_type THEN
    RAISE EXCEPTION 'B1_ACTION_TYPE_MISMATCH';
  END IF;
