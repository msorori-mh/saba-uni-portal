...
-- DRAFT ONLY — production apply is gated and requires explicit authorization.
--
-- Scope (forward-only):
--   1. student_trusted_devices  — device public keys (NO biometric material)
--   2. step_up_challenges       — short-lived, single-use challenges
--   3. step_up_proofs           — single-use proof tokens consumed by submit
--   4. issue_step_up_challenge / register_student_device / revoke_* RPCs
--   5. mint_step_up_proof (service_role only — called after server-side ECDSA verify)
--   6. submit_b1_student_request_atomic overload that consumes the proof
--      ATOMICALLY in the same transaction as the submit.
--
-- Invariants:
--   * No biometric image/template/score is stored anywhere. Only the device's
--     public key (SubjectPublicKeyInfo, base64 DER) is persisted.
--   * The client cannot self-authorize: proofs are minted only by service_role
--     after the server verifies the ECDSA signature over the canonical message
--     (version|challenge|nonce|user|device|action|request|payload_hash|expiry).
--   * The proof is bound to (user, device, action, request, payload_hash);
--     mutating the payload after confirmation invalidates it.
--   * No new RPC bypass: all existing authorization inside
--     submit_b1_student_request_atomic remains unchanged and authoritative.
