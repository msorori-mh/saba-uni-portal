...
--
-- PURPOSE
--   Create exactly 19 TEST_ONLY B1 fixture requests (104 runtime steps, 19
--   active steps, 5 transfer detail rows) so every one of the 22 previously
--   BLOCKED negative-matrix cases has a genuinely ACTIVE step to authorize
--   against, under the Migration-29 runtime-assignee identity contract.
--
-- REMEDIATION 15 — what changed versus design 13
--   G1 runtime steps copy the resolved assignment's SINGULAR identity kind
--      (user_id / staff_profile_id / faculty_profile_id /
--      position_assignment_id) instead of collapsing everything into
--      assigned_user_id; metadata.direct_assignment_id pins provenance.
--   G2 transfer_request_details rows are inserted BEFORE any transfer runtime
--      step (source = IT, target = CS; CIS is never stored).
--   G3 department actor model: source IT head, target CS head, unrelated CIS.
--   G4 deterministic request ids, runtime step ids and detail ids (no
--      gen_random_uuid()), so the negative MATRIX can bind at source level.
--
...
DECLARE
  ...
  k_dept_cis        CONSTANT uuid := '22222222-2222-4222-8222-222222222222'; -- UNRELATED (never stored)
  k_prog_it         CONSTANT uuid := '97638001-87cd-4df0-abe9-63c829504072'; -- current program (IT)
  k_prog_cs         CONSTANT uuid := '8df96335-4197-4e33-85ca-a970608f6a63'; -- requested program (CS)

  -- effective auth users of the direct assignments (identity is NOT stored as
  -- these values; they are only used to prove the expected actor).
...
BEGIN
  ...
  -- department/program scope model: IT source, CS target, CIS unrelated
  ...
  --      exactly one EFFECTIVE assignment per (unit, role[, dept]) using the
  --      same predicate as assert_b1_runtime_step_row_assignee_effective, and
  --      exactly one identity column non-null on that assignment.
  -- ======================================================================
  FOR r IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    -- department-scoped assignments must be position assignments in the exact department
    ...
    -- effective auth user must equal the expected test actor, and the
    -- underlying principal must be active
  ...
  LOOP
    ...
    -- ------------------------------------------------------------------
    -- G2 — transfer details BEFORE any transfer runtime step is inserted.
    -- The authorization contract reads department scope from this table, NOT
    -- from form_data. CIS is never stored as source or target.
    -- ------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------
    -- runtime steps: orders < active_order = completed (fixture
    -- initialization, no decision, no actor, no academic effect);
    -- active_order = active; orders > active_order = pending.
    -- ------------------------------------------------------------------
    FOR v_step IN
    ...
    LOOP
      -- G1 — resolve the department scope exactly like the production contract
      v_dept_scope := NULL;
      ...
      -- G1 — exactly one effective assignment, same predicate as production
      SELECT count(*) INTO v_n
        FROM public.request_processing_assignments a
      ...
      -- department-head steps must carry a position-assignment identity
      ...
      -- expected actor per the G3 department actor model
      ...
      -- G4 deterministic runtime step id
      ...
      INSERT INTO public.student_request_workflow_steps (
        ...
        -- G1: copy the assignment's singular identity kind verbatim
    ...
    -- exactly one active step per fixture request
  ...
  -- deterministic ids: 104 distinct step ids, 19 distinct request ids
  ...
  -- exactly one identity column per runtime step, and provenance pinned
  SELECT count(*) INTO v_n
    FROM public.student_request_workflow_steps s
    JOIN public.student_requests req ON req.id = s.student_request_id
   WHERE req.internal_notes = k_marker
  ...
  -- runtime identity must equal the pinned assignment identity
  SELECT count(*) INTO v_n
    FROM public.student_request_workflow_steps s
    JOIN public.student_requests req ON req.id = s.student_request_id
  ...
  -- every ACTIVE fixture step must satisfy the production activation contract
  FOR v_step IN
  ...
  -- transfer scope must be IT -> CS, and CIS must never be stored
  SELECT count(*) INTO v_n FROM public.transfer_request_details d
...
-- Matching cleanup: B1-FIVE-SERVICES-SAFE-RPC-FIXTURES-CLEANUP-13.NOT_APPLIED.sql
-- ============================================================================
