-- P1-09 — forward-only fix.
-- public.p1_apply_final_result_decision inserted into public.audit_logs using
-- non-existent columns (table_name, record_id, action, user_id), so every
-- registrar "apply final result" call failed with 42703. Production audit_logs
-- columns are (entity_type, entity_id, action_type, actor_user_id, new_values).
-- Behaviour is otherwise byte-identical to the deployed body.
