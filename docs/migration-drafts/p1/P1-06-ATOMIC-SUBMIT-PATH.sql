-- =====================================================================
-- P1-06 — ATOMIC SUBMIT PATH (forward-only draft, NOT APPLIED)
-- Mission: PORTAL_REFORM_P1_ATOMIC_SUBMIT_PATH_SOURCE_CLOSURE_07A
--
-- Canonical capability:
--   public.submit_student_request_with_details(
--     p_request_type text,
--     p_title        text,
--     p_form_data    jsonb,
...
--   ) RETURNS uuid
--
-- ONE contract serving BOTH:
--   A. normal authenticated student submission once student_visible = true
--   B. tightly gated TEST_ONLY submission while student_visible = false
--
-- Guarantees:
--   * eligibility -> request -> detail row -> submit -> workflow runtime,
--     all in the single transaction of the RPC call (any failure = full rollback)
--   * student identity derived only from auth.uid() + p1_active_student_profile
--   * generic create_student_request fails closed for the three P1 types
--   * no P1 request can reach 'submitted' without its canonical detail row
--   * B1-88 helpers and the five-service allowlist are NOT touched
-- =====================================================================
...
--    Rows are provisioned out-of-band by an operator (service_role).
--    RLS enabled with NO policies + no table grants to anon/authenticated:
--    the registry is readable only through SECURITY DEFINER predicates.
-- ---------------------------------------------------------------------
...
-- Server-side TEST_ONLY identity. NEVER derived from client JSON.
-- Fail-closed rule: the authenticated account must carry the approved
-- TEST_ONLY account convention already used by portal E2E accounts
-- (local part starts with 'test-only.' on the institutional domain).
...
-- Hidden-path predicate: ALL of (P1 service, TEST_ONLY actor, live run row
-- bound to this exact auth.uid() and service, unclaimed, in window).
...
BEGIN
  ...
  IF v_run NOT LIKE public.p1_e2e_07_marker() || '%' THEN
    RETURN NULL;
  END IF;
  -- Identity proof is server-side; knowing the marker is never sufficient.
...
BEGIN
  ...
  -- Actor binding: never from client input.
  ...
  -- ============ PRE-INSERT VALIDATION / SERVER RECOMPUTATION ==========
  -- Runs BEFORE any row exists, so duplicate-open checks can never observe
  -- the request currently being created (no self-detection), and every
  -- rejection happens before the first write.
  IF v_type.code = 'october_exam_entry_form' THEN
    -- canonical form field: remaining_courses (study_plan_courses ids)
    ...
    -- Authoritative server recomputation (level, remaining, eligible set).
  ...
  ELSE
    -- canonical form field: final_result_id === student_enrollments.id
    ...
    -- ownership + published + 7-day window + duplicate, all server-side
  ...
  -- ===================== OCTOBER EXAM ENTRY =========================
  -- All values below were recomputed server-side before the insert above.
  IF v_type.code = 'october_exam_entry_form' THEN
...
-- service_role intentionally NOT granted: it must never substitute for an
-- authenticated student, neither in production nor in E2E.

-- ---------------------------------------------------------------------
-- 4. CLOSE THE GENERIC CREATE BYPASS (create_student_request)
--    Full production body preserved; the only change is the P1 guard.
-- ---------------------------------------------------------------------
...
--    (a) submit_student_request explicit guard
--    (b) table-level trigger covering every other write path
-- ---------------------------------------------------------------------
...
--    The atomic RPC is SECURITY DEFINER (table owner) and bypasses RLS,
--    so removing the client-write branches has no functional regression.
-- ---------------------------------------------------------------------
