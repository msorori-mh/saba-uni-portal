...
-- ---------------------------------------------------------------------
-- 4. Exact active processing binding. This supplements direct identity;
--    it never substitutes for the direct runtime assignee.
-- ---------------------------------------------------------------------
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $function$
  SELECT auth.uid() IS NOT NULL AND (
    ...
    JOIN public.position_assignments pa ON pa.id = s.assigned_position_assignment_id
      AND pa.user_id = auth.uid() AND pa.is_active = true
      AND pa.assigned_from <= CURRENT_DATE
      AND (pa.assigned_to IS NULL OR pa.assigned_to >= CURRENT_DATE)
    JOIN public.request_processing_assignments rpa
      ON rpa.position_assignment_id = pa.id
...
-- ---------------------------------------------------------------------
-- 5. Closed B1 step tuple contract. A direct assignment on a malformed
--    unit/role/action tuple is never sufficient authorization.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 6. can_current_user_act_on_step: strict gate. No admin/registrar skip.
