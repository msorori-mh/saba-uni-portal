-- ============================================================================
-- PORTAL-B1-FIVE-SERVICES-SAFE-RPC-FIXTURE-PACKAGE — REMEDIATION 15
...
-- Targeting is by pinned deterministic id range AND the fixture marker;
-- nothing else can ever be matched by this script.
--
-- FORBIDDEN AND ABSENT HERE: any delete outside the fixture id ranges, any
...
DECLARE
  ...
  k_step_lo  CONSTANT uuid := 'f1300001-0000-4000-8000-000000000000';
  k_step_hi  CONSTANT uuid := 'f1300001-0000-4000-8000-999999999999';
  k_det_lo   CONSTANT uuid := 'f1300002-0000-4000-8000-000000000000';
  k_det_hi   CONSTANT uuid := 'f1300002-0000-4000-8000-999999999999';
  v_ids      uuid[];
  v_n        integer;
  v_rows     integer;
BEGIN
  ...
  -- refuse if any fixture step escaped the deterministic id space
  SELECT count(*) INTO v_n FROM public.student_request_workflow_steps
   WHERE student_request_id = ANY(v_ids)
     AND id NOT BETWEEN k_step_lo AND k_step_hi;
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'CLEANUP13_PRECONDITION_FAIL: % fixture steps outside deterministic id space', v_n;
  END IF;
  ...
  DELETE FROM public.transfer_request_details
   WHERE request_id = ANY(v_ids) AND id BETWEEN k_det_lo AND k_det_hi;
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows <> 5 THEN
    RAISE EXCEPTION 'CLEANUP13_FAIL: deleted % transfer detail rows (expected 5)', v_rows;
  END IF;

  DELETE FROM public.student_request_workflow_steps
   WHERE student_request_id = ANY(v_ids) AND id BETWEEN k_step_lo AND k_step_hi;
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows <> 104 THEN
  ...
  END IF;
  SELECT count(*) INTO v_n FROM public.student_request_workflow_steps
   WHERE id BETWEEN k_step_lo AND k_step_hi;
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'CLEANUP13_POSTFAIL: % fixture runtime steps remain', v_n;
  END IF;
  SELECT count(*) INTO v_n FROM public.transfer_request_details
   WHERE id BETWEEN k_det_lo AND k_det_hi;
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'CLEANUP13_POSTFAIL: % fixture transfer detail rows remain', v_n;
  END IF;
  ...
  RAISE NOTICE 'CLEANUP13_OK: 19 requests / 104 runtime steps / 5 transfer details removed';
END
$cleanup$;
