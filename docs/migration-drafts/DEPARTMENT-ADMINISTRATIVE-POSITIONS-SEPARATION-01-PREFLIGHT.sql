...
-- PostgreSQL 17 forbids CREATE TEMP TABLE in a read-only transaction.
-- Keep this preflight literal-read-only by materializing checks as CTE rows only.
WITH separation_preflight_checks(check_name, ok, detail) AS (
  VALUES
  ('three_departments',(
...
BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM separation_preflight_checks WHERE NOT ok
  ) THEN
