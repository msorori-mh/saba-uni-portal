...
- **Write governance (authoritative via PR339):** `PRODUCTION_WRITER=LOVABLE_ONLY` · `SINGLE_WRITER_LEASE=REQUIRED` · `OTHER_AGENTS=READ_ONLY` · `NEXT_PRODUCTION_WRITE=GA3_ONLY` · actionable `NEXT_WRITE=NONE_SCHEMA` while GA3 is VERIFIED_PRESENT
...
- **Status:** C0–C9 + GA1–GA3 production lineage proven; do not re-apply. Schema actionable next write is NONE_SCHEMA under lease.
