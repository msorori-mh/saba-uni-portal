# التقرير ممتاز، وهو فعليًا حسم لنا الصورة. **لا نحتاج إعادة بناء GP أو GA**؛ المتبقي أصبح صغيرًا ومحددًا.

بالنسبة لـ **مشاريع التخرج**: المنظومة التشغيلية والسياسات والـPinning والإنفاذ موجودة بالفعل. المتبقي الحقيقي فقط أن نتوقف عن الاعتماد على القيم التقنية المخترعة. لذلك أعتمد الترتيب التالي:

```text
GP-1
إعداد سياسة أكاديمية رسمية كـ Draft
↓
نراجع نحن القيم
↓
Publish
...
GP-2
المشاريع الجديدة:
policy_id = Published Policy
policy_pin_source = PUBLISHED_POLICY
policy_snapshot = immutable

GP-3
إلغاء BUILTIN DEFAULTS من gp_effective_policy
↓
لا توجد سياسة منشورة = FAIL CLOSED

GP-4
Regression + Authorization + E2E

```

**لا أنصح الآن بتحويل دورة حياة GP نفسها إلى Workflow Configurable.** التدقيق أثبت أنها Domain Kernel ثابتة وآمنة، وهذا مناسب حاليًا. أهم شيء ألا يستطيع المستخدم العبث بتسلسل أكاديمي حساس لمجرد أننا نملك Workflow Engine.

هناك قرار واحد يجب ألا تتخذه Lovable تلقائيًا: قيم السياسة الأكاديمية، خصوصًا:

```text
min_team_size
max_team_size
required_progress_reports
min/max_committee_members
passing_score
max_revision_rounds
proposal_window
defense_window
...
فنبقيهما كما هما حاليًا.

**ملاحظة مهمة:** المشاريع الستة الحالية مثبتة بالفعل على `BUILTIN_DEFAULT_AT_CUTOVER`. لا نغيرها الآن تلقائيًا إلى السياسة الجديدة؛ أولًا نحدد هل هي TEST_ONLY أم مشاريع حقيقية. السياسة الجديدة يجب أن تؤثر افتراضيًا على **المشاريع الجديدة فقط**.

---

أما **شؤون الخريجين** فالمتبقي أوضح:

```text
GA-1
Follow-up Type Catalog

GA-2
Versioned Follow-up Workflow

GA-3
Pin every new follow-up to workflow version

GA-4
Legacy existing followups
→ LEGACY_V1 snapshot

GA-5
Parity + Authorization Matrix
...
مثلًا بدل أن تكون هذه مكتوبة في UI:
...
تصبح سجلات إدارية من Catalog.

ثم يستطيع المسؤول تعريف:
...
بدون Migration جديدة.

لكن تبقى قواعد GA الحرجة ثابتة:

```text
approved graduate only
own record only
...
### إذن الحالة النهائية الآن

```text
Student Services Platform    ██████████  أساس Config-driven شبه مغلق
Graduation Projects Domain   ██████████  مكتمل تشغيليًا
GP Policy Configuration      ████████░░  ينقص القرار الأكاديمي + إزالة defaults
Graduates Affairs Domain     ██████████  مكتمل تشغيليًا
GA Continuity Policies       ██████████  Versioned بالفعل
GA Follow-up Configuration   ████░░░░░░  يحتاج Catalog + Workflow Versioning

```

وبما أن Lovable ما زالت تعمل على الإصلاحات السابقة، **لا أعطيها الآن مهمة تنفيذ جديدة متداخلة**. نخليها تنهي عملها الحالي، وبعدها نبدأ بالترتيب:

**أولًا GP policy closure، ثم GA follow-up configurability، ثم حملتي الاختبار الشامل للطالب والأكاديمي.**

وهذا أفضل بكثير من بدء الاختبارات الشاملة الآن ثم تغيير GP/GA بعدها وإعادة الاختبارات مرة ثانية.PORTAL-GP-GA-CURRENT-STATE-PRODUCTION-READONLY-AUDIT-01
