-- =====================================================================
-- B1-STAGE3 CLEANUP — DRY-RUN / EXECUTION SCRIPT (DOCUMENTATION ONLY)
-- Mission: B1_STAGE3_PREPARE_CLEANUP_EXECUTION_PREFLIGHT_PACKAGE_NO_EXECUTION-125
--
-- STATUS: **NOT EXECUTED**. This file is documentation. Running it requires a
-- separate, explicit owner DML approval.
--
-- Companions:
...
--   * explicit ID lists only — no LIKE mass delete, no TRUNCATE
--   * children first; never cascade from student_profiles
--   * single transaction; aborts on ANY precondition/postcondition mismatch
--   * touches NO storage object
--   * touches NO evidence / HOLD / legacy / non-TEST_ONLY row
--   * changes NO request_types.student_visible
--   * touches NO enrollment_certificate data
--   * performs NO DDL and NO migration
-- =====================================================================
...
-- Operator must set this deliberately after the byte-level storage export
-- decision is made (see preflight §6). Attachment METADATA rows are deleted
-- here; storage OBJECTS are not.
\set require_storage_export_acknowledged false

-- ---------------------------------------------------------------------
-- 0. Candidate sets (frozen, explicit)
-- ---------------------------------------------------------------------
...
-- Review the NOTICE output, then commit. Any RAISE EXCEPTION above aborts the
-- transaction automatically and leaves the database untouched.
COMMIT;
-- ROLLBACK;  -- use instead of COMMIT for a true dry run

-- ---------------------------------------------------------------------
-- 4. STANDALONE VERIFICATION QUERIES (read-only, run after commit)
-- ---------------------------------------------------------------------
-- 4.1 evidence + HOLD still present
-- SELECT request_number, request_type, status FROM student_requests
--  WHERE request_number IN ('SR-20260727-78427CC5','SR-20260727-50BEDCE2','SR-20260727-88D885F0',
--                           'SR-20260727-40E3E66B','SR-20260727-42393846','SR-20260727-3C550070',
--                           'SR-20260727-695EC35B','SR-20260727-F67CF366') ORDER BY 1;  -- expect 8
--
-- 4.2 protected legacy still present
-- SELECT request_number FROM student_requests
--  WHERE request_number IN ('SR-20260713-2DE64041','SR-20260715-FEDCB3E1','SR-20260716-26BAD4C8'); -- expect 3
--
-- 4.3 real student data untouched
-- SELECT count(*) FROM student_profiles WHERE academic_number NOT LIKE 'TEST_ONLY\_B1\_%';  -- unchanged
-- SELECT count(*) FROM student_requests r JOIN student_profiles p ON p.id = r.student_profile_id
--  WHERE p.academic_number NOT LIKE 'TEST_ONLY\_B1\_%';                                      -- unchanged
--
-- 4.4 visibility unchanged
-- SELECT code, student_visible FROM request_types
--  WHERE code IN ('enrollment_suspension','excused_absence','department_transfer','final_chance',
--                 'file_withdrawal','enrollment_certificate') ORDER BY 1;
--
-- 4.5 enrollment_certificate untouched
-- SELECT count(*) FROM enrollment_certificate_document_details;
-- SELECT count(*) FROM enrollment_certificate_document_generation_attempts;
--
-- 4.6 migration head unchanged
-- SELECT max(version) FROM supabase_migrations.schema_migrations;  -- expect 20260730175527
--
-- 4.7 storage untouched (metadata rows for evidence only; 20 objects now orphaned by design)
-- SELECT count(*) FROM student_request_attachment_uploads;         -- expect 8
-- =====================================================================
