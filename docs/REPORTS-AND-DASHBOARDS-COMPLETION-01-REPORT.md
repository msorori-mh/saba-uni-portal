...
## 3.3 المخاطر المتبقية المقبولة (threat model)

1. **zero-pinning:** عندما يصبح متبقي معادلات الجمع المنشورة R=0 تُستنتج الخلايا المحجوبة خارج أعمدة KPI الظاهرة = 0 بالضبط — أي كشف «غياب نشاط» على مستوى الدور/الخلية فقط (لا قيمة غير صفرية تُكشف ولا فرد). قيد متأصل في أي نظام كتمان خلايا ينشر إجمالاً كلياً — موجود أيضاً في by_status وفي عقد graduates-affairs المدموج — وإزالته تتطلب interval protection خارج النطاق ⇒ خطر متبقٍّ مقبول موثق.
2. **distinct_roles:** قيمته المحجوبة تُستنتج تافهةً من عدد صفوف by_role الظاهرة بنية الجدول نفسه — استنتاج تافه لا معلومة إضافية فيه.
3. **ملاحظة إتاحة:** شرط «الصف كامل الظهور» لهوامش by_role قد يجعل by_role محجوبة غالباً في الإنتاج — كلفة إتاحة متحفظة مقصودة لصالح عدم الاسترجاع.
