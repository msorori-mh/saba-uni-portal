...
- **الفرع:** `feat/graduates-affairs-completion-01` من `main` عند `8f229d09`
...
2. **D-13 = NEEDS_USER_INPUT:** صُممت كسياسة configurable (`AccountContinuityPolicy`) افتراضها
   ...
   fail-closed (ترفض غير المعتمدة/خارج الصلاحية/غير المدرج، والقدرات الحساسة `portal_sign_in`
   و`university_email_reuse` تتطلب flags مخصصة إضافةً للإدراج).
3. **التقارير مجمعة/غير شخصية:** `buildCohortEmploymentReports` و`aggregateSurveyResponses`
   و`graduate_aggregate_employment_report` كلها تمر عبر حجب الخلايا الصغيرة
   (`GREATEST(minimum, 3)`، افتراضي 5)، والمدخلات answers-only/aggregate-only بنيوياً، مع
...
- `docs/migration-drafts/GRADUATES-AFFAIRS-MVP-COMPLETION-01.sql`
  ...
  - `evaluate_graduate_account_continuity(text,text,timestamptz)` و
    `graduate_aggregate_employment_report(uuid,integer,integer)` — SECURITY DEFINER بـ
    `search_path` مثبت، REVOKE من PUBLIC/anon/authenticated، RLS default-deny بلا سياسات.
  - يعتمد على مسودة الأساس (يعيد استخدام `reject_graduate_immutable_mutation`) ولا يعدّلها.

**اختبارات (2):**
- `tests/graduates-affairs/graduates-affairs-completion-01.test.ts` — 22 اختبار unit
...
- `bun test tests/graduates-affairs/` → **28/28 ناجح** (6 أساس + 22 استكمال).
- تنفيذ فعلي على PostgreSQL 18.4 معزول (نفس نمط تحقق الأساس):
  ...
  - سلسلة الاستكمال (setup → FOUNDATION-01 → COMPLETION-01 → pg-verify) → **OK**:
    ...
    الخلفية/بعد الإنهاء، منع تعديل هوية المتابعة؛ قبول رسالة موافق عليها ورفض موافقة مسحوبة
    ونقطة ملغاة/غير موثقة، append-only للسجل؛ سياسة undecided ترفض، الموافقة بلا provenance
    مرفوضة، السياسة المعتمدة تسمح بالقدرة المدرجة فقط وتمنع الحساسة بلا flag، الصف المُقرّر
    غير قابل للتعديل/الحذف، المنتهية/المرفوضة/المفقودة ترفض؛ تقرير الأفواج يحجب تحت العتبة
    ويحسب فوقها بدقة (3/2/2/2)، الافتراضي 5 يحجب 3، الأحداث المُحال عليها لا تُحسب، والسجلات
    الملغاة تخرج من العينة؛ RLS/ACL default-deny على الجداول الثلاثة والدالتين.
